...
| 상황 | 액션 |
|------|------|
| 제안이 **버그 수정** (실제 동작 오류) | ✅ 즉시 적용 필수 |
| 제안이 **보안/로버스트성 개선** | ✅ 즉시 적용 필수 |
| 제안이 **스타일/최적화** | 검토 후 판단 |

**금지 사항**: "나중에 수정하겠다" 는 허용하지 않음. PR에 포함시킬 수 없으면 PR 자체를 생성하지 않습니다.
...
PR에 리뷰 피드백(Qodo, 사람 리뷰 모두)이 달리면 **수정 커밋 + 답글**을 반드시 함께 수행합니다:

1. **리뷰 확인**: `gh api repos/{owner}/{repo}/pulls/{pr}/comments`로 피드백 내용을 확인합니다.
2. **수정 커밋**: 피드백을 반영하여 수정 후 PR 브랜치에 push합니다.
3. **PR 답글 작성**: PR 코멘트에 반영 내역을 답글로 작성합니다:
   - 반영 커밋 해시 명시
...
**원칙**: 커밋만 push하고 답글을 생략하지 않습니다. 리뷰어가 어떤 피드백이 반영되었는지 추적할 수 있어야 합니다.
