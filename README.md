...
1. **자동 파이프라인 (`/auto-generate`)** — 사전 대화(Phase 0) → Drive 동기화 → 에이전트 리서치 → 검증 → Cascade 업데이트 → 완료 보고까지 5단계 자동 실행. 유일한 중단점: Phase 0 사용자 인터뷰 및 Google 로그인
...
8. **스펙 템플릿 (v2.0)** — Product Spec·Design Spec·Tech Spec 책임 분리 구조. Product Spec이 IA·플로우·외부연동(EXT)의 단일 소스, Design Spec은 emocog 디자인 시스템 규칙(항목 1-11) 중심, Tech Spec은 LLM 최소 가드레일(항목 1-4) 중심; **화면정의서 가이드** — Product Spec §3-2 화면 목록의 각 화면(SCR-N)을 10개 항목(User Goal·System Goal·Data Model·Information Hierarchy·UI Components·Interactions·Error States·Empty States·Accessibility·Constraints)으로 상세화하는 기준서; **§3-3 화면별 상세 정의** — Product Spec 템플릿에 포함된 화면별 상세 정의 섹션(H4 화면명 + H5 항목 구조). Synth 에이전트가 product-spec 생성 시 화면정의서 가이드의 10개 항목을 자동 적용(P0: 전체 필수, P1/P2: 1~5 필수)
9. **PR/Issue 템플릿** — `.claude/templates/`의 PR·Issue 표준 형식 보장
10. **프로젝트 공유 (`/share-project`)** — 생성된 문서와 메타데이터를 PR로 팀에 공유
11. **Drive 업로드 (`/upload-drive`)** — 생성된 문서를 Google Drive에 HTML 서식 유지하여 업로드
12. **관리자 모드 (`/admin`)** — 템플릿 maintainer 전용. 요구사항 → 플랜 → 구현 → 검증 → PR 자동 생성
13. **자동 마이그레이션** — 템플릿 업데이트 시 세션 시작 시 자동으로 스키마 마이그레이션 실행
14. **Worktree 기반 브랜치 격리** — 여러 세션이 동시 작업해도 브랜치 충돌 없음
15. **MVP Kill Gate 검토 (`/gate-review`)** — S1~S5 단계별 완료 기준을 항목별로 검토하고 Go/Pivot/Kill 결정을 기록
16. **출력물 검증 (`/verify`)** — 생성된 문서의 구조·스키마·인용·완전성을 자동 검증. debate/critique 파일(`{active_product}/{doc_type}/v{N}/agents/debate/`, `{active_product}/{doc_type}/v{N}/agents/critique.json`)을 포함한 모든 에이전트 산출물 경로를 검사
17. **S3 프로토타입 구현 (`/implement`)** — 완성된 스펙 3종(Product Spec·Design Spec·Tech Spec) 최신 버전을 자동 탐지하여 구현 에이전트에 전달. 최초 구현/re-implement 모드 자동 판단, bkit 환경 감지 분기, `--force` 플래그 지원
...
사용자: "자동으로 만들어줘"
         ...
         ▼
┌─────────────────────┐
│Phase 0: 사전 대화     │  템플릿 섹션 기반 인터뷰 — 사용자 입력/이전 문서/연구 분류
└─────────┬───────────┘
          ▼
┌─────────────────────┐
...
└─────────┬───────────┘
          ▼
┌─────────────────────┐
│Phase 3.5: cascade   │  Stage Doc → Product Brief 업데이트 (S1~S3)
│                     │  Design/Tech Spec → Product Spec 역반영 (S3)
└─────────┬───────────┘
          ▼
┌─────────────────────┐
...
| S3 | Prototype | 반복 | "내부 시연이 준비되었는가?" | - |
| S4 | Freeze | 0.5W | "Freeze & 인계 가능한가?" | - |
| S5 | MVP | 3.5W | "P0 기능이 출시 가능한가?" | - |
...
| S3 | Design Spec | `design-spec` | Handoff Doc |
| S3 | Tech Spec | `tech-spec` | Handoff Doc |
...
**MVP 프로세스 문서 (권장)**: `product-brief`, `business-spec`, `pretotype-spec`, `product-spec`, `design-spec`, `tech-spec` — S1~S3 단계에서 자동 결정됩니다.
