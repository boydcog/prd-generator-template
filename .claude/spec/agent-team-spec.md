...
출력 경로: `.claude/artifacts/{active_product}/agents/debate/judgment.json` + `.claude/artifacts/{active_product}/agents/debate/summary.md`
...
- 미해결 질문 종합
- **문서 구조 규칙** (절대 준수):
  - **허용**: 각 섹션(H2/H3) 내부의 본문 내용은 분석 결과에 맞게 충분히 추가 가능
  - **허용**: `[내용]`, `[항목]` 형식의 플레이스홀더를 실제 내용으로 교체
  - **허용**: 헤더 텍스트 내 `[프로젝트명]` 같은 인라인 플레이스홀더 교체
  - **금지**: H2 섹션 삭제, 섹션 순서 변경, 새로운 H2 섹션 추가
  - **금지**: 표(table) 행/열 구조 변경
  - **원칙**: 템플릿의 H2/H3 헤더 구조·순서는 반드시 유지. 내부 본문은 증거·분석 기반으로 충분히 보강
...
> `{active_product}`는 런타임에 `.claude/state/_active_product.txt`에서 로드되는 활성 제품 ID입니다.

### 회의 (도메인 에이전트)

| 에이전트 | JSON (peer_discussions 포함) | Markdown |
|---------|------|----------|
| biz | `.claude/artifacts/{active_product}/agents/biz.json` | `.claude/artifacts/{active_product}/agents/biz.md` |
| marketing | `.claude/artifacts/{active_product}/agents/marketing.json` | `.claude/artifacts/{active_product}/agents/marketing.md` |
| research | `.claude/artifacts/{active_product}/agents/research.json` | `.claude/artifacts/{active_product}/agents/research.md` |
| tech | `.claude/artifacts/{active_product}/agents/tech.json` | `.claude/artifacts/{active_product}/agents/tech.md` |
| pm | `.claude/artifacts/{active_product}/agents/pm.json` | `.claude/artifacts/{active_product}/agents/pm.md` |
...
| 에이전트 | JSON | Markdown |
|---------|------|----------|
| critique | `.claude/artifacts/{active_product}/agents/critique.json` | `.claude/artifacts/{active_product}/agents/critique.md` |
| synth | `.claude/artifacts/{active_product}/{output_dir}/v{N}/{output_file}` | — |
| synth | `.claude/artifacts/{active_product}/{output_dir}/v{N}/citations.json` | — |
| synth | `.claude/artifacts/{active_product}/{output_dir}/v{N}/conflicts.json` | — |
