...
출력 경로: `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/debate/judgment.json` + `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/debate/summary.md`
...
3. **출력 경로**: `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/{role_id}.json` + `{role_id}.md` (`{active_product}`는 런타임에 `.claude/state/_active_product.txt`에서 로드, `{output_dir_name}`과 `v{N}`은 현재 실행 컨텍스트에서 결정)
...
모든 에이전트 로그는 **버전별 독립 경로** `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/`에 저장됩니다.
(이전 버전의 `agents/` 단일 경로는 더 이상 사용하지 않습니다.)

| 에이전트 | JSON (peer_discussions 포함) | Markdown |
|---------|------|----------|
| biz | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/biz.json` | `…/agents/biz.md` |
| marketing | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/marketing.json` | `…/agents/marketing.md` |
| research | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/research.json` | `…/agents/research.md` |
| tech | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/tech.json` | `…/agents/tech.md` |
| pm | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/pm.json` | `…/agents/pm.md` |
...
| 에이전트 | JSON | Markdown |
|---------|------|----------|
| critique | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/agents/critique.json` | `…/agents/critique.md` |
| synth | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/{output_file_name}` | — |
| synth | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/citations.json` | — |
| synth | `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/conflicts.json` | — |

> `{active_product}`는 `.claude/state/_active_product.txt`에서 로드합니다. `{output_dir_name}`, `{output_file_name}`은 `document-types.yaml`의 해당 문서 유형에서 결정됩니다.
