...
| 7 | `synth` | 통합/종합 | Wave 1.5(critique 포함) 결과 머지, 충돌 해결, 최종 문서 작성 (문서 유형에 따라 구조 결정) |
...
### Wave 1.5 — 순차 (Wave 1 완료 후, Wave 2 이전)

critique 에이전트가 Wave 1의 모든 결과(JSON + MD)를 읽고 비판적으로 검토합니다.
논리적 오류, 근거 없는 주장, 역할 간 모순, 누락된 관점을 식별하고 synth를 위한 개선 권고를 제공합니다.

### Wave 2 — 순차 (Wave 1.5 완료 후)

synth 에이전트가 Wave 1과 critique 결과를 읽고 최종 문서를 생성합니다.
