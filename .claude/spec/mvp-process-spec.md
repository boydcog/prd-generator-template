...
```
S1 Brief (1W) ─→ [Gate 1] ─→ S2 Pretotype (2W) ─→ [Gate 2 Kill Gate] ─→ S3 Prototype (반복) ─→ [Gate 3] ─→ S4 Freeze (0.5W) ─→ [Gate 4] ─→ S5 MVP (3.5W)
                                                                                    ↑_______________|
                                                                             (내부 시연 준비까지 반복)
```

**페이즈 구조**:
- **Discovery** (S1 1W + S2 2W): 시장 검증 → 시장 반응 검증
- **Iteration** (S3 반복): Product/Design/Tech Spec 작성 + 프로토타입 구현 반복, 내부 시연 준비 완료까지
- **Delivery** (S4 0.5W + S5 3.5W): Freeze & 인계 → MVP 빌드

| 단계 | 이름 | 기간 | 핵심 질문 | Kill Gate |
|------|------|------|-----------|-----------|
| S1 | Brief | 1W | "이 문제가 실재하는가?" | 아니오 |
| S2 | Pretotype | 2W | "시장이 이 솔루션에 반응하는가?" | **예** |
| S3 | Prototype | 반복 | "내부 시연이 준비되었는가?" | 아니오 |
| S4 | Freeze | 0.5W | "Freeze & 인계 가능한가?" | 아니오 |
...
| Design Spec | S3 | `DESIGN-SPEC.md` | UI/UX 화면 레이아웃·상호작용 명세 |
| Tech Spec | S3 | `TECH-SPEC.md` | 기술 아키텍처·AI 구현 가이드 |
...
Product Spec  ──→ Design Spec    (S3 AC·플로우 → S3 화면 명세)
Product Spec  ──→ Tech Spec      (S3 AC → S3 API 명세·기술 스택)
Design Spec + Tech Spec ──→ S4   (Freeze & 인계 인풋)
Design Spec + Tech Spec + Product Spec ──→ S5   (MVP 개발 인풋)
```
...
Product/Design/Tech Spec 기반 프로토타입 구현. 내부 시연 준비가 완료될 때까지 iteration 루프를 반복합니다.

### 핵심 활동

1. P0 기능 스코프 확정 (Pretotype Spec §5-4 인풋 기반)
2. 핵심 플로우 및 수용 기준(AC) 명세 → Product Spec 작성
3. 기술 리스크 식별 및 해소 → Tech Spec 작성
4. 모든 P0 화면 레이아웃 명세 → Design Spec 작성
5. 프로토타입 빌드 → 내부 시연
6. 피드백 반영 → Spec 업데이트 → 재빌드 (내부 시연 준비 완료까지 반복)
...
### Gate 3 기준 (`design-spec.gate_criteria`)

- [ ] Design Spec 항목 1-5 이모코그 표준 스택 확인 (기술 스택·토큰·컴포넌트·타이포·아이콘)
- [ ] Design Spec 항목 6-9 타겟 사용자 맞춤 조정 완성 (인터랙션·접근성·레이아웃·CSS 변수)
- [ ] Design Spec 항목 10-11 신규 작성 완료 (UX 전략·품질 검수 체크리스트)
- [ ] Tech Spec 항목 1 Core Stack 모든 레이어 선택 및 버전 명시 완료
- [ ] Tech Spec 항목 4 Key Integration — Product Spec §7-3 EXT 목록 반영 완료
- [ ] PM·디자이너·엔지니어 3자 합의
- [ ] 내부 시연 준비 완료 (PM·CEO·CTO 데모 승인)
...
내부 시연 킬게이트 통과 후 스펙 문서를 Freeze하고 Product Team에 인계합니다.

### 핵심 활동

1. S3에서 작성된 Product Spec / Design Spec / Tech Spec 최종 검토
2. 스코프 변경 금지(Freeze) 선언 — PM·디자이너·엔지니어 3자 합의
3. Product Team에 세 문서 전달 (인계 절차 완료 확인)
...
Product Team이 S4 인계 문서(Product Spec / Design Spec / Tech Spec) 기반으로 MVP를 실제 개발합니다.
...
| S3 Prototype | **Iterate** | Pretotype Spec §5-4 | Product Spec·Design Spec·Tech Spec 초안, P0 구현 (반복) | PM·CEO·CTO: 내부 시연 승인·Gate 3 판정 |
| S4 Freeze | **Handoff** | S3 완성 문서 | — (Freeze 선언 + Product Team 인계) | PM·디자이너·엔지니어: 3자 합의·Gate 4 판정 |
| S5 MVP | **Ship** | Design Spec + Tech Spec + Product Spec | 배포 가능한 P0 빌드 | PM: P0 기능 확인·버그 판정·출시 결정 |
...
| S2 | `pretotype-spec` | |
| S3 | `product-spec` | design-spec·tech-spec은 product-spec 생성 후 순차 생성 |

> **S4 Freeze**: 새 문서를 생성하지 않으므로 init-project에서 S4 선택 시 gate-review 바로 진행.
...
사용자: "S2 시작해줘"
  ...
  → /gate-review (S2 Kill Gate 판정)
    ├─ Go  → mvp_stage: S3, /auto-generate (Product Spec → Design Spec → Tech Spec 순차)
    │         [내부 시연 준비까지 Spec 업데이트 + 재빌드 반복]
    │         → /gate-review (S3 Gate 판정 — design-spec gate_criteria 기준)
    │           └─ Go → mvp_stage: S4
    │                   [Product Team에 3개 문서 전달 + Freeze 합의]
    │                   → /gate-review (S4 Gate 판정 — 고정 기준)
    │                     └─ Go → mvp_stage: S5, MVP 빌드 시작
