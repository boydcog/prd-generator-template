...
> **Disclaimer:** 이 문서는 LLM 코딩 도구가 최적 성능으로 개발할 수 있도록 **최소한의 가드레일**을 제공한다.
> 상세한 API 명세나 데이터 모델 설계는 LLM이 자율적으로 판단한다.
> 항목 2\~3은 이모코그 표준으로 prefill됨. 항목 1·4만 프로젝트별 작성.

---

## **1\. Core Stack**

> 후보군에서 프로젝트 성격에 맞게 선택. \[선택\] 표시된 셀을 채운다.
...
| 2 | **User 참조** | 사용자 식별은 JWT의 `sub` (subject) + `iss` (issuer) 조합으로 처리 |
| 3 | **네이밍 컨벤션** | DB 컬럼명은 `snake_case`. TypeScript 코드에서는 `camelCase`로 매핑 |
| 4 | **유연한 속성** | 선택적·확장 가능한 속성은 `JSONB` 타입으로 저장 (스키마 변경 최소화) |
| 5 | **개인정보 암호화** | 이메일, 전화번호 등 PII는 저장 전 암호화 필수 (AES-256 또는 Supabase Vault) |
...
| \# | 규칙 | 세부 내용 |
| ----- | ----- | ----- |
| 1 | **클라이언트 범위 최소화** | 렌더링 비용이 없는 컴포넌트는 서버 측 또는 정적으로 처리. Next.js: Server Components 기본, 인터랙션 필요 시에만 `"use client"`. Expo: 기본 함수형 컴포넌트, 불필요한 전역 상태 최소화 |
| 2 | **입력 검증** | 모든 API 입력에 Zod 스키마 검증 필수. `any` 타입으로 우회 금지 |
| 3 | **네이밍 컨벤션** | 컴포넌트/타입: `PascalCase`. 파일/폴더: `kebab-case`. DB 컬럼: `snake_case` |
| 4 | **타입 안전성** | `any`, `unknown` 사용 금지. TypeScript strict 모드 활성화 |
| 5 | **환경 변수** | API 키·비밀값 코드 하드코딩 금지. `.env.local` + `.env.example` 관리 |
| 6 | **DB 접근** | ORM 또는 Supabase Client 경유만 허용. Raw SQL 직접 실행 금지 |
| 7 | **에러 처리** | API 응답은 `{ data, error }` 구조로 통일. 서버 에러는 로깅 후 generic 메시지 반환 |

---

## **4\. Key Integration**

> **프로젝트별 필수 작성.** 이 제품에 필요한 외부 연동만 작성. 불필요한 항목 삭제.
...
| 3 | Design Spec §1 기술 스택과 항목 1 Core Stack 일치 확인 | \[ \] |
