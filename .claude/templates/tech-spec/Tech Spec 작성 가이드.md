...
| 방향 | 문서 | 읽는 섹션 |
| ----- | ----- | ----- |
| **인풋** | Product Spec §6 (엣지 케이스 & 제약) | 경계 조건 → §3 API 에러 처리, §6 보안 요구사항 |
| **인풋** | Product Spec §7 (데이터 & 외부 연동) | 7-1 데이터 소스 → §4 데이터 모델, **7-3 외부 연동 목록 → §3 API 명세 우선 작성 대상** |
| **인풋** | Product Spec §2 (사용자 스토리) | US → 구현 범위, AC → 테스트 케이스 기준 |
| **인풋** | Design Spec §5 (비주얼 시스템) | 기술 스택 확인 (shadcn + Tailwind + next/image) |
...
| ----- | ----- | ----- | :---: | :---: |
| 1 | **전제 정보** (기술 스택 & 전제 조건) | 무엇으로 만드는가? 전제 조건은? | **필수** | 확정 |
| 2 | **최소 연동 구조** | 핵심 연동 포인트는 무엇인가? | 골격 | **완성** |
| 3 | API 명세 | 어떤 API가 필요한가? (외부 연동 우선) | 목록 | **완성** |
| 4 | 데이터 모델 & 스키마 | 어떤 데이터를 어떻게 저장하는가? | 초안 | **완성** |
| 5 | AI Rules & 가드레일 | AI 코딩 도구에게 무엇을 지시하는가? | — | **필수** |
| 6 | 보안 & 성능 최소 기준 | 어떤 기본 기준을 충족해야 하는가? | 선택 | **필수** |
| 7 | 인프라 & 배포 | 어디에 어떻게 배포하는가? | 선택 | **필수** |
| 8 | 기술 리스크 & PoC 결과 | 검증된 기술 리스크는 무엇인가? | **필수** | 확정 |
| Gate | Gate 4 자가 점검 | 개발 시작 전 확인 사항 | — | **필수** |

## **1. 전제 정보 (기술 스택 & 전제 조건)**

이 섹션이 답하는 질문: "무엇으로 만드는가? AI 코딩 도구와 엔지니어가 작업 시작 전에 알아야 할 전제는?"
인풋: Product Brief §6 (스코프), Feasibility PoC 결과

**[재명칭 이유]** "기술 개요 & 스택"에서 "전제 정보"로 변경. AI 코딩 도구가 컨텍스트 로딩 시 가장 먼저 참조하는 섹션임을 명시.
...
AI 코딩 도구와 엔지니어가 작업 전 반드시 설정해야 하는 환경 조건.

| 항목 | 값 / 결정 | 비고 |
| ----- | ----- | ----- |
| **Node.js 버전** | \[버전 이상\] | `.nvmrc`에 고정 |
| **패키지 매니저** | \[npm / yarn / pnpm / bun\] | lock 파일 커밋 필수 |
| **환경 변수 로딩** | `.env.local` | 예시: `.env.example` |
| **DB 마이그레이션** | \[Prisma Migrate / 기타\] | 최초 실행 명령 |
| **주요 포트** | localhost:3000 (dev) | \[기타\] |
| **외부 서비스 전제** | Product Spec §7-3 연동 목록 기준 | \[EXT-001, EXT-002 사전 등록 필요\] |
...
| **외부 의존성** | Product Spec §7-3 EXT 목록 | \[대안 없는 의존성\] |
...
이 섹션이 답하는 질문: "핵심 연동 포인트는 무엇인가? 레이어 간 데이터가 어떻게 흐르는가?"

**[간소화 이유]** 전체 아키텍처 상세 서술이 아닌, AI 코딩 도구와 엔지니어가 알아야 할 최소 연동 구조만 기술. 레이어 책임과 핵심 데이터 흐름에 집중.
...
| **외부 서비스 레이어** | Product Spec §7-3 외부 연동 처리 | UI 로직 |

### **2-2. 핵심 데이터 흐름**

가장 중요한 P0 기능의 흐름만 기술. 세부 구현은 §3~§4에서 확장.
...
7. 서비스 → 결과 가공 → API Route 반환
8. API Route → JSON 응답 → UI 업데이트
```

### **2-3. 외부 연동 연결 요약**

Product Spec §7-3 외부 연동 목록을 내부 레이어와 매핑.

| 연동 ID | 서비스명 | 호출 레이어 | 인증 방식 | 실패 시 폴백 |
| ----- | ----- | ----- | ----- | ----- |
...
인풋: Product Spec §7-3 (외부 연동 목록) + §2 (사용자 스토리)

**[우선순위]** Product Spec §7-3 외부 연동 목록(EXT-00N)을 우선 명세. 각 EXT-00N이 어떤 API 엔드포인트로 구현되는지 매핑 관계를 명시한다.

### **3-1. API 엔드포인트 목록**

| Method | Path | 설명 | 인증 필요 | 관련 US | 연동 EXT |
| ----- | ----- | ----- | ----- | ----- | ----- |
...
> **Product Spec §7-3 매핑 확인:** 위 `연동 EXT` 열이 Product Spec §7-3의 EXT-00N과 누락 없이 매핑되었는가?

### **3-2. 주요 엔드포인트 상세**

#### **POST /api/\[리소스\]**

**목적:** \[이 API가 하는 일\] | **연동:** \[EXT-00N 또는 내부\]
...
이 섹션이 답하는 질문: "어떤 기본 기준을 충족해야 하는가?"
인풋: Product Spec §6 (엣지 케이스 & 제약)
**[간소화 이유]** 기본 요구사항만 기재. 고도화는 MVP 이후 별도 스펙으로 분리.
...
| \[EXT-001_API_KEY\] | \[외부 서비스 키 — EXT-001 대응\] | 필수 | `sk-...` |
| \[변수명\] | \[설명\] | 선택 | \[예시\] |
...
Product Spec §7-3과 동일 목록. 장애 시나리오 추가.

| 서비스 (EXT-ID) | 용도 | 대안 | SLA |
| ----- | ----- | ----- | ----- |
| \[서비스\] (EXT-001) | \[용도\] | \[대안 없음/있음\] | \[가용성\] |
...
- [ ] Product Spec §7-3 외부 연동 목록 → §3 API 명세 1:1 매핑 확인
...
| **Product Spec 정합** | §3 API 명세의 EXT 연동이 Product Spec §7-3 목록과 1:1 매핑되는가? | \[Pass/Fail\] |
