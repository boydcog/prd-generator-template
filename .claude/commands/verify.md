...
2. `document-types.yaml`에서 해당 유형의 `output_sections[]`를 로드합니다.
   - **`output_sections`가 빈 배열(`[]`)인 경우 (MVP 문서 유형의 기본 동작)**:
     로컬 템플릿 파일에서 섹션 목록을 동적으로 파싱합니다:
     1. `document-types.yaml`의 `output_dir_name`을 읽어 `.claude/templates/{output_dir_name}/` 디렉토리를 확인합니다.
     2. 해당 디렉토리에서 `[프로젝트명]`으로 시작하는 `.md` 파일을 찾습니다.
     3. 파일을 읽어 `##` 레벨 헤더(H2)를 모두 추출하여 섹션 목록으로 사용합니다.
     4. 로컬 템플릿 파일도 없으면: "섹션 목록을 확인할 수 없습니다. 완전성 검사를 건너뜁니다." 경고 후 PASS.
3. 각 섹션이 최종 문서에 포함되어 있는지 확인합니다:
   - 비교 전 **양쪽 헤더 문자열을 정규화**합니다: 마크다운 강조(`**`, `*`, `__`), 이스케이프(`\.`, `\-` 등), 선행/후행 공백을 제거한 후 비교합니다.
   - 정규화된 섹션 제목을 문서 내 헤딩(#, ##)에서 검색합니다.
   - 누락된 섹션을 보고합니다.
