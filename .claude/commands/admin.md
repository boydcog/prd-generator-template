...
### Step 7: 로컬 검증 완료 보고 및 공유 여부 확인

로컬에서 모든 검증이 완료되면 결과를 보고하고 **사용자에게 공유 여부를 묻습니다**.

1. 변경 내역 요약을 표시합니다:
   - 수정된 파일 목록
   - 변경 내용 한 줄 요약
2. 사용자에게 공유 여부를 확인합니다:

   **개발자(GitHub 용어에 익숙한 경우)**:
   > "로컬 검증이 완료되었습니다. 변경 내용을 PR로 올릴까요?"

   **비개발자(GitHub 용어가 낯선 경우)**:
   > "로컬에서 모두 확인되었습니다. 이 변경 내용을 팀원에게 검토받으러 보낼까요? 보내면 팀원이 검토 후 반영할 수 있습니다."

   사용자가 개발자인지는 `.user-identity` 또는 대화 맥락으로 판단합니다. 모호하면 비개발자용 표현을 사용합니다.

3. 사용자 응답에 따라:
   - **"네" / "올려줘" / "보내줘"** → Step 8 실행
   - **"아니요" / "나중에" / "아직"** → "로컬에 저장되어 있습니다. 준비되면 말씀해주세요." 안내 후 종료

### Step 8: Worktree PR 생성 (사용자 승인 후)
...
1. PR URL을 표시합니다 (개발자) 또는 "팀원에게 검토 요청을 보냈습니다." 안내합니다 (비개발자).
2. 변경 요약을 보고합니다.
