...
| C6 | Download 이벤트 트리거 | export URL은 파일 다운로드를 트리거. `res.text()`로 내용을 읽을 수 없음. `page.goto(exportUrl)`는 "Download is starting" 오류 발생 → **사용 금지**. 앵커 클릭 방식만 사용 | `waitForEvent('download')`를 먼저 생성한 뒤 **앵커 클릭**(`page.evaluate` 내 `a.click()`)으로 트리거 필수 |
...
browser_run_code:
  async (page) => {
    ...
    const download = await downloadPromise;
    const tempPath = await download.path();   // playwright-artifacts 내 임시 파일 경로
    return tempPath;
  }
→ 반환값을 tempPath 변수로 받은 뒤 Bash tool: cp "{tempPath}" ".claude/knowledge/{active_product}/evidence/raw/{target_filename}"
```
