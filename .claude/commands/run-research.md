...
1. **project.json 존재**: `.claude/state/{active_product}/project.json`이 있는지 확인. 없으면 `/init-project`를 먼저 실행하라고 안내.
2. **증거 인덱스 존재**: `.claude/knowledge/{active_product}/evidence/index/sources.jsonl`이 있는지 확인. 없으면 `/sync-drive`를 먼저 실행하라고 안내.
...
1. `.claude/artifacts/{active_product}/{output_dir_name}/` 디렉토리에서 기존 버전 폴더를 확인합니다 (`v1/`, `v2/`, ...).
...
```
.claude/artifacts/{active_product}/{output_dir_name}/
├── v1/
│   ├── {output_file_name}
│   ├── citations.json
│   ├── conflicts.json
...
### Step 0.65: 이전 단계 문서 로드 (MVP 단계 문서인 경우)

`project.json`의 `mvp_stage`를 확인합니다.
`mvp_stage`가 S2 이상이면 이전 단계 완성 문서를 **주 컨텍스트**로 로드합니다:

| 현재 단계 | 주 참조 문서 (artifacts에서 최신 버전 로드) |
|----------|----------------------------------------|
| S2 | `.claude/artifacts/{active_product}/product-brief/v{latest}/` + `.claude/artifacts/{active_product}/business-spec/v{latest}/` |
| S3 | `.claude/artifacts/{active_product}/pretotype-spec/v{latest}/` |
| S4 (design-spec) | `.claude/artifacts/{active_product}/product-spec/v{latest}/` |
| S4 (tech-spec) | `.claude/artifacts/{active_product}/product-spec/v{latest}/` |
| S1 또는 null | 이 단계를 건너뜀 (Drive 청크가 주 입력) |

- `{latest}`: 해당 디렉토리에서 가장 높은 버전 번호 (`v1/`, `v2/`, ... 중 최신)를 자동 감지합니다.
- 이전 단계 문서가 있으면 에이전트 프롬프트의 **"이전 단계 문서 (핵심 컨텍스트)"** 섹션에 포함합니다.
- 이전 단계 문서가 없으면 (첫 실행 또는 파일 미존재): 이 단계를 건너뜁니다.
...
2. 각 청크를 키워드/주제로 분류하여 역할별 관련 청크를 분배합니다:
   ...
   - 분류 불가 → 모든 에이전트에 전달
3. **Step 0.65에서 이전 단계 문서를 로드한 경우**:
   - 이전 단계 문서에서 이미 다루어진 영역은 Drive 청크 분배를 최소화합니다.
   - 이전 단계 문서에서 언급된 미해결 질문, 가정, 리스크 영역을 중심으로 청크를 분배합니다.
   - Drive 청크는 "보조 참고 자료"로 위치시킵니다.
4. 에이전트 프롬프트에 파일 경로 대신 **사전 조합된 텍스트**를 전달합니다.
...
각 에이전트에게 전달할 컨텍스트:
- `.claude/state/{active_product}/project.json` (프로젝트 설정)
- `.claude/knowledge/{active_product}/evidence/index/sources.jsonl` (증거 인덱스)
- `.claude/knowledge/{active_product}/evidence/chunks/` (증거 청크 파일들)
...
3. **템플릿 배치 (선행 작업)**:
   - 로컬 템플릿 파일 경로: `.claude/templates/{output_dir_name}/` 디렉토리에서 `[프로젝트명]`으로 시작하는 `.md` 파일을 찾습니다.
   - 출력 디렉토리가 없으면 생성: `.claude/artifacts/{active_product}/{output_dir_name}/v{N}/`
   - 템플릿 파일을 출력 경로에 **그대로 복사**합니다:
     - 원본: `.claude/templates/{output_dir_name}/[프로젝트명]*.md`
...
3-1. **draft-inputs 로드 (사전 대화 결과)**:
   ...
   - 존재하면 로드하여 섹션별 출처(`user` / `prior` / `research`)와 내용을 파악합니다.
...
7. **전문가 토론 요약 섹션 작성**: `debate/summary.md` 기반으로 토론 핵심 내용 요약
   - 이 섹션은 **템플릿의 마지막 섹션 이후에 추가**합니다 (`## 부록 E. 전문가 토론 요약` 또는 유사 제목으로 문서 말미에 Append).
   - 템플릿 원본 H2 섹션들 사이에 삽입하지 않습니다. "불변 규칙"의 '추가 금지'는 템플릿 원본 섹션 구간에만 적용되며, 문서 말미 Append 섹션은 예외입니다.
8. **통합 문서 채우기 (섹션별 순차 작성)**:
   - Step 3에서 출력 경로에 배치된 파일을 섹션별로 순차적으로 채웁니다.
   - **불변 규칙 (절대 위반 금지)**:
     - H1/H2/H3 헤더의 **섹션 레벨(##, ###)과 구조**는 변경/추가/삭제하지 않습니다.
       단, 헤더 텍스트 내 `\[...\]` 인라인 플레이스홀더(예: `\[프로젝트명\]`)는 교체 대상에 포함됩니다.
     - 표(table) 행/열 구조는 변경하지 않습니다.
     - `\[내용\]`, `\[항목\]` 형식의 플레이스홀더를 실제 내용으로 교체합니다.
   - **작업 순서**:
     a. 파일을 읽어 모든 플레이스홀더 위치를 식별합니다.
     b. Judge `adopted_for_synth` 내용이 반영되어야 하는 섹션을 먼저 처리합니다.
     c. 각 섹션을 위에서 아래로 순서대로 처리합니다.
     d. 파일을 읽어 모든 플레이스홀더를 채운 뒤 Write 도구로 한 번에 저장합니다. 저장 후 H2/H3 헤더 구조와 표 행/열이 원본 템플릿과 동일한지 확인합니다.
     e. 채울 증거/데이터가 없는 플레이스홀더는 `[데이터 없음]`으로 표시합니다.
   - 동적 역할의 관점은 관련 섹션의 해당 행에 자연스럽게 통합합니다.
...
## 이전 단계 문서 (핵심 컨텍스트, 있는 경우 최우선 참조)
Step 0.65에서 로드한 이전 단계 완성 문서입니다.
이 문서가 있을 때는 아래 Drive 청크보다 우선하여 주 컨텍스트로 활용하세요.
이전 단계 문서에서 미해결로 남겨진 질문, 가정, 리스크를 중점적으로 보완하세요:
{이전 단계 문서 내용 또는 "없음 (S1 단계 또는 이전 문서 미존재)"}

## 증거 자료 (보조 참고 자료)
Step 0.7에서 이 역할에 관련된 증거만 선별하여 전달합니다.
이전 단계 문서가 있을 때는 보조 참고 자료로, 없을 때는 주 입력으로 사용합니다:
{역할별 사전 조합된 증거 텍스트}
...
이전 단계 문서가 있을 때는 보조 참고 자료로 활용합니다:
{역할별 사전 조합된 증거 텍스트}
...
## 통합 규칙
1. **섹션 출처 우선순위**: `.claude/state/{active_product}/draft-inputs.json`이 있으면 반드시 로드하세요. 출처별 처리:
...
- 인용 보고서: .claude/artifacts/{active_product}/{output_dir_name}/v{N}/citations.json
- 충돌 보고서: .claude/artifacts/{active_product}/{output_dir_name}/v{N}/conflicts.json
- 메타데이터: .claude/artifacts/{active_product}/{output_dir_name}/v{N}/metadata.json
