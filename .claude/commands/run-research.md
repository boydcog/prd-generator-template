...
4. **문서 구조**: 출력 경로(`.claude/artifacts/{active_product}/{output_dir_name}/v{N}/{output_file_name}`)에 이미 템플릿이 복사되어 있습니다. 이 파일을 직접 편집하여 플레이스홀더를 채우세요. 새 파일을 처음부터 작성하지 마세요. H1/H2/H3 헤더와 표 구조는 유지하고, `[내용]` 형식의 플레이스홀더만 교체하세요.
