...
- improve: Product Spec §3-3 화면별 상세 정의 섹션 신설 + Synth 에이전트 화면정의서 가이드 연결 — `[프로젝트명] Product Spec.md`에 §3-3 화면별 상세 정의(10개 항목: User Goal·System Goal·Data Model·Information Hierarchy·UI Components·Interactions·Error States·Empty States·Accessibility·Constraints) 플레이스홀더 섹션 추가, 기존 §3-3 네비게이션 패턴을 §3-4로 번호 변경; `run-research.md` Synth 프롬프트 입력 파일에 product-spec 전용 `화면정의서 가이드.md` 참조 추가 + 통합 규칙 #9(화면정의서 작성) 신설; `Product Spec 작성 가이드.md`에 §3-3 화면별 상세 정의 작성 원칙(P0/P1/P2 필수항목 표) 추가, §3-3→§3-4 번호 변경
