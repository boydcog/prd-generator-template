...
- improve: S3 `/implement` 커맨드 신설 + iteration 현황 표시 — `startup.sh` S3 스펙 완성 시 추천 액션을 `gate-review` → `implement`로 변경(S3은 구현 먼저, gate는 내부 시연 후), `implementation.status=complete` 시 `gate-review`로 자동 전환; 세션 시작 시 `문서 버전 현황` 블록 추가(artifacts 폴더 스캔, `grep || true`로 pipefail 안전 처리, `agents/` 폴더 제외); `.claude/commands/implement.md` 신규 생성 — 스펙 3종 canonical 파일명(`PRODUCT-SPEC.md` 등) 기반 자동 탐지·최초구현/re-implement 모드 자동 판단·`project.json` spec_refs 업데이트·bkit 감지 분기·`--force` 플래그 지원; `CLAUDE.md` §2에 `implement` 추천 액션 케이스 추가, §5에 "구현해줘"/"스펙 업데이트됐어" 자연어 트리거 추가, §5-1 기능 목록에 implement 추가; `README.md` 핵심 기능 #17에 `/implement` 항목 추가 (Qodo 리뷰 반영: Bug #2/#3/#4, Rule #1) ([`2d5455a`](https://github.com/boydcog/prd-generator-template/commit/2d5455a))

- improve: S3 Iteration 단계 재정의 + sync-drive 수정일 기반 사전 필터링 — `mvp-process-spec.md` S3을 "반복 루프(내부 시연 준비까지)" 단계로 재정의: Design Spec·Tech Spec을 S4→S3으로 이동, Gate 3 기준을 design-spec gate_criteria 기반으로 통합, AI Agent 맵 S3/S4 역할 업데이트, 단계 전환 시퀀스 iteration 루프 표현 추가; S4를 "Freeze & 인계" 단계로 재정의: 신규 산출물 없음, Gate 4 고정 기준 2개(Product Team 인계·Freeze 합의); `document-types.yaml` design-spec·tech-spec `mvp_stage` S4→S3 변경, description 업데이트; `gate-review.md` S3 단계명 "Prototype (Iteration)" 변경, S3 gate criteria를 design-spec 기준으로 수정, S4 Freeze 고정 게이트 기준 섹션 신설; `startup.sh` get_stage_docs() S3에 design-spec·tech-spec 추가, S4를 빈 문자열(신규 문서 없음)로 변경 + STAGE_DOCS 빈 경우 STAGE_COMPLETE=true 처리; `sync-drive.md` Step 0.5 수정일 기반 사전 필터링 신설 — ledger scraped_at 대비 Drive modifiedTime 비교, 변경/신규/확인불가 소스만 재수집 대상으로 선별, --refresh 플래그로 전체 강제 재수집; 이슈 #57은 PR #58에서 이미 수정됨 확인 후 closed (issues [#59](https://github.com/boydcog/prd-generator-template/issues/59), [#54](https://github.com/boydcog/prd-generator-template/issues/54), closes [#57](https://github.com/boydcog/prd-generator-template/issues/57)) ([`3e76995`](https://github.com/boydcog/prd-generator-template/commit/3e76995))

- improve: Product Spec §3-3 화면별 상세 정의 섹션 신설 + Synth 에이전트 화면정의서 가이드 연결 — `[프로젝트명] Product Spec.md`에 §3-3 화면별 상세 정의(10개 항목: User Goal·System Goal·Data Model·Information Hierarchy·UI Components·Interactions·Error States·Empty States·Accessibility·Constraints, H4/H5 헤딩 계층) 플레이스홀더 섹션 추가, 기존 §3-3 네비게이션 패턴을 §3-4로 번호 변경; `run-research.md` Synth 프롬프트 입력 파일에 product-spec 전용 `화면정의서 가이드.md` 참조 추가 + 통합 규칙 #9(화면정의서 작성) 신설; `Product Spec 작성 가이드.md`에 §3-3 화면별 상세 정의 작성 원칙(P0/P1/P2 필수항목 표) 추가, §3-3→§3-4 번호 변경; `README.md` 핵심 기능 #8에 §3-3 화면별 상세 정의 설명 추가 ([`17b0b89`](https://github.com/boydcog/prd-generator-template/commit/17b0b89))
- improve: Design Spec 템플릿 v3 — Static/Dynamic 이중 계층 구조 전면 재편, Human Touch 체크리스트(항목 11) 신설, 항목 순서 재정렬(접근성→6, 레이아웃→7, 인터랙션→8); 기억생생 예시 시니어 CSS 토큰 + UX 전략·Human Touch 체크리스트 테이블 형식으로 교체; 가이드 v3.0 — Static vs Dynamic 계층 설명, 3단계 추론(항목 10), Human Touch 도출 방법(항목 11) 상세화; run-research.md design-spec 전용 Step 3-A 추가 — Context7로 gluestack-ui v2/Tailwind v4/Framer Motion 라이브러리 문서 페칭하여 항목 9 CSS 변수 생성 시 API 문법 검증에 활용
- improve: 화면정의서 가이드 추가 — `.claude/templates/product-spec/화면정의서 가이드.md` 신규 생성(10개 항목: User Goal·System Goal·Data Model·Information Hierarchy·UI Components·Interactions·Error States·Empty States·Accessibility·Constraints, Part 1 프레임워크 개요 + Part 2 항목별 작성 가이드 + Part 3 빈 템플릿); `Product Spec 작성 가이드.md` §3 정보 구조 앞에 화면정의서 가이드 참조 블록 추가; Qodo 리뷰 반영 — 2.3 Data Model §4→§2 수정, 2.7 Error States §6→§4 수정, README 핵심 기능 #8 화면정의서 가이드 설명 추가 ([`6db4b38`](https://github.com/boydcog/prd-generator-template/commit/6db4b38), [`753c964`](https://github.com/boydcog/prd-generator-template/commit/753c964))
