...
- improve: S3 `/implement` 커맨드 신설 + iteration 현황 표시 — `startup.sh` S3 스펙 완성 시 추천 액션을 `gate-review` → `implement`로 변경(S3은 구현 먼저, gate는 내부 시연 후); 세션 시작 시 `문서 버전 현황` 블록 추가(artifacts 폴더 스캔 → `product-spec: v2 (2회)` 형태 출력); `.claude/commands/implement.md` 신규 생성 — 스펙 3종 최신 버전 자동 탐지·최초구현/re-implement 모드 자동 판단·`project.json` spec_refs 업데이트·bkit 감지 분기·`--force` 플래그 지원; `CLAUDE.md` §2에 `implement` 추천 액션 케이스 추가, §5에 "구현해줘"/"스펙 업데이트됐어" 자연어 트리거 추가, §5-1 기능 목록에 implement 추가 ([`d602d96`](https://github.com/boydcog/prd-generator-template/commit/d602d96))
