...
- improve: 화면정의서 가이드 추가 — `.claude/templates/product-spec/화면정의서 가이드.md` 신규 생성(10개 항목: User Goal·System Goal·Data Model·Information Hierarchy·UI Components·Interactions·Error States·Empty States·Accessibility·Constraints, Part 1 프레임워크 개요 + Part 2 항목별 작성 가이드 + Part 3 빈 템플릿); `Product Spec 작성 가이드.md` §3 정보 구조 앞에 화면정의서 가이드 참조 블록 추가; Qodo 리뷰 반영 — 2.3 Data Model §4→§2 수정, 2.7 Error States §6→§4 수정, README 핵심 기능 #8 화면정의서 가이드 설명 추가
